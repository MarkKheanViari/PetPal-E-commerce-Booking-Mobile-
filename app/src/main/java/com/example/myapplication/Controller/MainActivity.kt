...
class MainActivity : AppCompatActivity() {
    ...
    private fun fetchProducts() {
        val url = "http://192.168.168.55/backend/fetch_product.php"
        ...
        client.newCall(request).enqueue(object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                try {
                    ...
                    val baseImageUrl = "http://192.168.168.55/backend/uploads/"
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                try {
                    ...
                    for (i in 0 until productsArray.length()) {
                        ...
                        val rawImage = productJson.optString("image", "").trim()
                        val fullImageUrl = if (rawImage.isNotEmpty() && !rawImage.startsWith("http")) {
                            "http://192.168.168.55/backend/uploads/$rawImage"
                        } else {
                            rawImage
                        }
                        val finalImageUrl = if (fullImageUrl.isNotEmpty()) fullImageUrl else "http://192.168.168.55/backend/uploads/default.jpg"
