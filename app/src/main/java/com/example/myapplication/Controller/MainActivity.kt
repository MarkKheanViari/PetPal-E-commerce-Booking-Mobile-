...
class MainActivity : AppCompatActivity() {
    ...
    private fun checkForApprovedAppointments(userId: Int) {
        val url = "http://192.168.137.14/backend/fetch_approved_appointments.php?mobile_user_id=$userId"
    ...
    private fun fetchProducts() {
        val url = "http://192.168.137.14/backend/fetch_product.php"
        ...
        client.newCall(request).enqueue(object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                try {
                    ...
                    val baseImageUrl = "http://192.168.137.14/backend/uploads/"
    ...
    private fun fetchProductsByCategory(category: String) {
        ...
        val url = "http://192.168.137.14/backend/fetch_product.php?category=$category"
        val request = Request.Builder().url(url).build()
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                try {
                    ...
                    for (i in 0 until productsArray.length()) {
                        ...
                        val rawImage = productJson.optString("image", "").trim()
                        val fullImageUrl = if (rawImage.isNotEmpty() && !rawImage.startsWith("http")) {
                            "http://192.168.137.14/backend/uploads/$rawImage"
                        } else {
                            rawImage
                        }
                        val finalImageUrl = if (fullImageUrl.isNotEmpty()) fullImageUrl else "http://192.168.137.14/backend/uploads/default.jpg"
